...
class OccurrencesDao {
    ...
    companion object {
        ...
        fun searchOccurrences(): ArrayList<String> {
            ...
            for (i in result.result.documents.indices) {
                val coord = result.result.documents[i].get("coordinates").toString()
                val long = StringUtils.getLonDB(coord)
                val lat = StringUtils.getLatDB(coord)
                arraylist.add((result.result.documents[i].get("type").toString() + ";" + lat + ";" + long))
            }
            return arraylist
        }
